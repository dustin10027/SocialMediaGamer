...
public class ProfileFragment extends Fragment {
    ...
    private void checkIfExistPost() {
        mListener= mPostProvider.getPostByUser(mAuthProvider.getUid()).addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
            public void onEvent(@Nullable QuerySnapshot queryDocumentSnaphots, @Nullable FirebaseFirestoreException error) {
                if(queryDocumentSnaphots !=null){
                    int numberPost = queryDocumentSnaphots.size();
                    if(numberPost > 0){
